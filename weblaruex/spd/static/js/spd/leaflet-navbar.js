...
(function() {
  L.Control.NavBar = L.Control.extend({
    options: {
      position: 'topleft',
      center:[39.1666700, -6.1666700], 
      zoom : 8,
      bbox: layer_zonas_met_aemet, //Alternative to center/zoom for home button, takes precedence if included
      forwardTitle: 'Avanzar en el historial de la vista del mapa',
      backTitle: 'Retroceder en el historial de la vista del mapa',
      homeTitle: 'Ir a la vista de inicio del mapa',
      zoomInTitle: 'Zoom in',
    ...
    onAdd: function(map) {
      ...
      // Add toolbar buttons
      this._zoomInButton = this._createButton(options.zoomInTitle, controlName + '-zoom-in', container, this._goZoomIn);
      this._homeButton = this._createButton(options.homeTitle, controlName + '-home', container, this._goHome);
      this._zoomOutButton = this._createButton(options.zoomOutTitle, controlName + '-zoom-out', container, this._goZoomOut);
      ...
      try {
        this._map.fitBounds(this.options.bbox.getBounds());
      } catch(err){

...
from .models import Estaciones, Eventos, UmbralesRios, UltimosValores, UltimasImagenes, SucesosInundacion, InformesTrimestrales, Canales, AdjuntosEventos, DocumentacionAdjunto, RelacionEstacionesCamaras
import locale
...
def getHorariosSol(request):
    ...
    peticionHttp = requests.get(
        'http://api.sunrise-sunset.org/json?lat=39.16667&lng=-6.16667&formatted=0'
    )
...
def getCieloAemet(request):
    ...
    peticionHttpAemet = requests.get(
        'http://www.aemet.es/es/api-eltiempo/variables/eCielo/PB/7/' +
        now.strftime('%Y-%m-%dT%H') + ':00:00+02:00/01/D+' + str(cont) + '/70')
    while len(peticionHttpAemet.text) == 0 and cont < 10:
        cont = cont + 1
        peticionHttpAemet = requests.get(
            'http://www.aemet.es/es/api-eltiempo/variables/eCielo/PB/7/' +
            now.strftime('%Y-%m-%dT%H') + ':00:00+02:00/01/D+' + str(cont) +
            '/70')
...
def getImagenesPreAcumAemet(request):
    peticionHttpAemet = requests.get(
        'http://www.aemet.es/es/api-eltiempo/modelo-harmonie/timeline/61/PB/1%20hora'
    )
    result = peticionHttpAemet.json()
    return JsonResponse(result, safe=False)
...
def getImagenesRadar(request):
    peticionHttpAemet = requests.get(
        'http://www.aemet.es/es/api-eltiempo/radar/timeline/RN1/PB')
    result = peticionHttpAemet.json()
    return JsonResponse(result, safe=False)
...
def getDocumentsEventos(request):
    ...
    if request.method == 'POST':  # 1.
        ...
        if form.is_valid():  # 2.1
            ...
            tipoFechaEvento = info.get(
                "tipoFecha"
            )  # get tipo de fecha del evento (inicio: para crear un nuevo evento, fin: para cerrar el evento)
            tituloEvento = info.get("titulo")  # get titulo del evento
            fechaEvento = datetime.strptime(
                info.get("fecha"),
                '%d/%m/%Y')  # get fecha (inicio o fin) del evento

            if tipoFechaEvento == "inicio":  # Si tipo de fecha = inicio ... se quiere crear un nuevo evento
                if Eventos.objects.using('spida_web').filter(
                        fecha_hora_inicio=fechaEvento
                ).exists(
                ):  # Compruebo si ya existe un evento con la misma fecha de inicio. Si existe ...
                    print("Ya existe un evento con fecha de inicio",
                          fechaEvento)

                    eventos = Eventos.objects.using('spida_web').order_by(
                        'estado', '-fecha_hora_inicio'
                    )  #obtengo la lista de eventos actualizados
                    return render(
                        request, "docu-eventos.html", {
                            "user":
                            user,
                            "Eventos":
                            eventos,
                            "form":
                            FormFechaIniFinEvento(),
                            "MensajeError":
                            "Ya existe un evento con fecha " +
                            info.get("fecha")
                        })

                else:  # Si no existe ....
                    print("Se va a crear un nuevo evento con fecha de inicio",
                          fechaEvento)
                    new_entry = Eventos(
                        titulo=tituloEvento,
                        fecha_hora_inicio=fechaEvento,
                        estado='0'
                    )  # Creo la query para almacenarlo en la base de datos
                    new_entry.save(
                        using='spida_web'
                    )  # Guardo el nuevo evento en la base de datos
                    print(
                        "El nuevo evento ha sido cargado en la base de datos de forma satisfactoria"
                    )
                    ...
                    return render(
                        request, "docu-eventos.html", {
                            ...
                            FormFechaIniFinEvento(),
                            "MensajeSuccess":
                            'Ha sido creado un nuevo evento con fecha ' +
                            info.get("fecha")
                        })

            elif tipoFechaEvento == "fin":  # Si tipo de fecha = fin ... se quiere dar por terminado un evento ya existente

                query = Eventos.objects.using('spida_web').filter(
                    id_evento=idEvento)
                if query.exists():  # Si el evento existe en la base de datos
                    if (query.first().fecha_hora_inicio).replace(
                            tzinfo=None
                    ) <= fechaEvento:  # Y ademas la fecha de finalizacion del evento es mayor o igual a la fecha de inicio
                        Eventos.objects.using('spida_web').filter(
                            id_evento=idEvento
                        ).update(
                            titulo=tituloEvento,
                            fecha_hora_fin=fechaEvento,
                            estado='1'
                        )  # Le añado la fecha fin del evento y cambio su estado ('0': Abierto, '1':Cerrado)
                        ...
                        return render(
                            request, "docu-eventos.html", {
                                "user": user,
                                "Eventos": eventos,
                                "form": FormFechaIniFinEvento(),
                                "MensajeSuccess":
                                'Evento de inundación concluido'
                            })
                        ...
                        return render(
                            request, "docu-eventos.html", {
                                ...
                                FormFechaIniFinEvento(),
                                "MensajeError":
                                "La Fecha de finalización del evento de inundación no puede ser menor que la fecha de comienzo de éste"
                            })
                else:
                    eventos = Eventos.objects.using('spida_web').order_by(
                        'estado', '-fecha_hora_inicio'
                    ...
                    return render(
                        request, "docu-eventos.html", {
                            ...
                            FormFechaIniFinEvento(),
                            "MensajeError":
                            "No se puede cerrar el evento con ID " + idEvento +
                            " porque no se encuentra registrado en la base de datos"
                        })
            ...
            descripcionAdjunto = info.get(
                "descripcion")  # get descripcion del adjunto
            fecha_hora = datetime.now()

            #Añado el nuevo adjunto
            Evento = Eventos.objects.using('spida_web').filter(
                id_evento=int(idEvento)).first()
            new_entry = AdjuntosEventos(
                id_evento=Evento,
                titulo_adjunto=tituloAdjunto,
                id_usuario=user.id,
                fecha_hora_local=fecha_hora,
                descripcion=descripcionAdjunto
            )  # Creo la query para almacenarlo en la base de datos
            new_entry.save(using='spida_web'
                           )  # Guardo el nuevo evento en la base de datos
            idAdjunto = new_entry.id_adjunto

            listFicherosAdjuntos = request.FILES.getlist(
                'docspida')  # get ficheros adjuntados

            for fichero in listFicherosAdjuntos:  # recorro los ficheros adjuntos
                print("Fichero: ",
                      fichero.name.split('.')[0],
                      fichero.name.split('.')[-1], "Size:", fichero.size)
                nombrefichero = (fichero.name.split('.')[0]).replace(
                    " ", "_")  # nombre del fichero
                extfichero = fichero.name.split(
                    '.'
                )[-1]  # extension del fichero (tipo: Ejemplo pdf, doc, jpg etc.)
                sizefichero = fichero.size  # tamaño del fichero (En bytes)
                CargarFicheroAdjuntado(
                    fichero, idEvento, idAdjunto, nombrefichero, extfichero,
                    sizefichero
                )  # Almaceno cada fichero en media/SPIDA y en la base de datos
            ...
            return render(
                request, "docu-eventos.html", {
                    ...
                    "Eventos": eventos,
                    "form": FormFechaIniFinEvento(),
                    "MensajeSuccess": 'Se ha adjuntado nueva documentación'
                })
            ...
            eventos = Eventos.objects.using('spida_web').order_by(
                'estado', '-fecha_hora_inicio')
            return render(request, "docu-eventos.html", {
                "user": user,
                "Eventos": eventos,
                "form": FormFechaIniFinEvento()
            })
...
def CargarFicheroAdjuntado(fichero, idEvento, idAdjunto, nombreFichero,
                           extensionFichero, sizeFichero):
    fecha_hora = datetime.now()
    rutaFichero = '/SPIDA/EIS_' + str(idEvento).zfill(
        4
    ) + '_' + fecha_hora.strftime("%Y%m%d%H%M%S") + nombreFichero + '.' + str(
        extensionFichero)  # ruta donde va a ser guardado el fichero adjuntado
    ...
    with open(settings.MEDIA_ROOT + rutaFichero, 'wb+') as destination:
        ...
        Adjunto = AdjuntosEventos.objects.using('spida_web').filter(
            id_adjunto=int(idAdjunto)).first()
        new_entry = DocumentacionAdjunto(
            id_adjunto=Adjunto,
        ...
        new_entry.save(
            using='spida_web')  # Guardo el nuevo evento en la base de datos
...
def getSucesosSpida(request):
    user = request.user
    sucesos = SucesosInundacion.objects.using('spida_web').order_by(
        '-fecha_hora_local')
    ...
    for suceso in sucesos:
        ...
        sucesoAuxiliar['fecha'] = (
            suceso.fecha_hora_local).strftime("%d %B, %Y")
        sucesoAuxiliar['imagen'] = base64.b64encode(suceso.imagen).decode()
        listaSucesos.append(sucesoAuxiliar)

    return render(request, "sucesosSpida.html", {
        "user": user,
        "Sucesos": listaSucesos
    })
...
def getInformesTrimestrales(request):
    ...
    if request.method == 'POST':
        ...
        if 'docInformeTrimestral' in request.FILES:  # 2.2
            ...
            info = request.POST  # get info del POST
            numCuatrimentre = int(
                info.get("cuatrimentre")
            )  # get cuatrimentre del informe valorado (1 (Enero a Marzo), 2 (Abril a Junio), 3 (Julio a Septiembre) o 4 (Octubre a Diciembre))
            yearCuatrimestre = int(info.get("year"))
            first_date = datetime(yearCuatrimestre, 3 * numCuatrimentre - 2, 1)
            last_date = datetime(yearCuatrimestre + 3 * numCuatrimentre // 12,
                                 3 * numCuatrimentre % 12 + 1,
                                 1) + timedelta(days=-1)
            print("Cuatrimentre: ", numCuatrimentre, "Fecha Ini: ", first_date,
                  "Fecha fin: ", last_date)

            #Añado el nuevo informe trimestral a la base de datos
            if InformesTrimestrales.objects.using('spida_web').filter(
                    fecha_hora_local_inicio=first_date,
                    fecha_hora_local_fin=last_date).exists():
                print("Ya existe un informe para este cuatrimestre")
                return render(request, "informesSpida.html", {"user": user})
            else:
                InformeTrimestral = request.FILES.getlist(
                    'docInformeTrimestral')[0]  # get pdf adjuntado
                nombreInformeTrimestral = (InformeTrimestral.name).replace(
                    " ", "_")  # nombre del fichero
                sizeInformeTrimestral = InformeTrimestral.size  # tamaño del pdf (En bytes)
                rutaInformeTrimestral = '\SPIDA\INFORMES_TRIMESTRALES\ITS_' + str(
                    yearCuatrimestre) + '_' + str(
                        numCuatrimentre) + 'C' + '.pdf'

                print(InformeTrimestral, nombreInformeTrimestral,
                      sizeInformeTrimestral, rutaInformeTrimestral)

                with open(settings.MEDIA_ROOT + rutaInformeTrimestral,
                          'wb+') as destination:
                    print("Escribiendo fichero")
                    for chunk in InformeTrimestral.chunks():
                        destination.write(chunk)

                new_entry = InformesTrimestrales(
                    id_cuatrimestre=numCuatrimentre,
                    year=yearCuatrimestre,
                    fecha_hora_local_inicio=first_date,
                    fecha_hora_local_fin=last_date,
                    nombre=nombreInformeTrimestral,
                    size=sizeInformeTrimestral,
                    ruta=rutaInformeTrimestral,
...
def getListadoInformesTrimestrales(request):
    ...
    listInformesTrimestrales = InformesTrimestrales.objects.using(
        'spida_web').order_by('-fecha_hora_subida').extra(
            select={
                'id': 'id_informe',
            ...
            }).values('id', 'cuatrimestre', 'year', 'fecha_ini', 'fecha_fin',
                      'size', 'path', 'fecha_subida')
...
def OpenInformeTrimestral(request):
    ...
    contenido = InformesTrimestrales.objects.using('spida_web').filter(
        id_informe=idInformeTrimestral)[0]
    return FileResponse(open(settings.MEDIA_ROOT + contenido.ruta, 'rb'),
                        content_type='application/pdf')  #force-download
...
def DownloadDocuEventos(request, idDoc):
    user = request.user
    documento = DocumentacionAdjunto.objects.using('spida_web').filter(
        id_documento=idDoc)[0]

    return FileResponse(open(settings.MEDIA_ROOT + documento.ruta, 'rb'),
                        as_attachment=True,
...
def OpenProcoloto(request):
    ...
    return FileResponse(open(settings.MEDIA_ROOT + urlProtocolo, 'rb'),
...
def getEstaciones(request):
    ...
    if user.is_anonymous:
        estaciones = UmbralesRios.objects.using('spida_web').filter(
            id_estacion__visualizar=1).filter(
                id_estacion__monitorizar=1).filter(id_estacion__id_red=1)
        ultimos_valores = UltimosValores.objects.using('spida_web').filter(
            id_canal=100).filter(id_estacion__visualizar=1).filter(
                id_estacion__monitorizar=1).filter(id_estacion__id_red=1)
    else:
        estaciones = UmbralesRios.objects.using('spida_web').filter(
            id_estacion__visualizar=1).filter(id_estacion__monitorizar=1)
        ultimos_valores = UltimosValores.objects.using('spida_web').filter(
            id_canal=100).filter(id_estacion__visualizar=1).filter(
    ...
    for estacion in estaciones:
        ...
        estacionAuxiliar["N1"] = round(
            estacion.limite_n1 * (estacion.limite_desbordamiento / 100), 2)
        estacionAuxiliar["N2"] = round(
            estacion.limite_n2 * (estacion.limite_desbordamiento / 100), 2)
        estacionAuxiliar["N3"] = round(
            estacion.limite_n3 * (estacion.limite_desbordamiento / 100), 2)
        ...
        for valor in ultimos_valores:
            if valor.id_estacion.id_estacion == estacion.id_estacion.id_estacion:
                ...
                locale.setlocale(locale.LC_TIME, 'es_ES.UTF-8')
                estacionAuxiliar["fecha_utc"] = valor.fecha_hora_utc.strftime(
                    "%d %B a las %H:%M h")
                estacionAuxiliar[
                    "fecha_local"] = valor.fecha_hora_local.strftime(
                        "%Y-%m-%d %H:%M:%S")  #.strftime("%d %B a las %H:%M h")
                break

        fechaActual = (datetime.now() +
                       timedelta(hours=-2)).strftime("%Y-%m-%d %H:%M:%S")

        if estacionAuxiliar["fecha_local"] != None and estacionAuxiliar[
                "valor"] != None and estacionAuxiliar[
                    "N1"] > 0 and estacionAuxiliar[
                        "N2"] > 0 and estacionAuxiliar["N3"] > 0:
            #if estacionAuxiliar["fecha_local"]<(datetime.now()+timedelta(hours=-2)).strftime("%d %B a las %H:%M h"):
            if datetime.strptime(estacionAuxiliar["fecha_local"],
                                 "%Y-%m-%d %H:%M:%S") < datetime.strptime(
                                     fechaActual, "%Y-%m-%d %H:%M:%S"):
            ...
            estacionAuxiliar["fecha_local"] = (datetime.strptime(
                estacionAuxiliar["fecha_local"],
                "%Y-%m-%d %H:%M:%S")).strftime("%d %B a las %H:%M h")
    ...
    cols = [
        'id', 'nombre', 'red', 'widget', 'N1', 'N2', 'N3', 'valor',
        'fecha_utc', 'fecha_local', 'estado'
    ]
...
def getMiniaturasSpida(request):
    ...
    for imagen in imagenes:
        ...
        imagenAuxiliar['fecha_hora'] = imagen.fecha_hora_local.strftime(
            "%d %B a las %H:%M h")
...
def getImagenSpida(request):
    ...
    imagen = UltimasImagenes.objects.using('spida_web').filter(
        id_estacion=idEstacion, id_camara=idCamara).first()
    if imagen is not None:
        result['estacion'] = imagen.id_estacion.nombre
        result['camara'] = imagen.id_camara
        result['fecha_hora'] = imagen.fecha_hora_local.strftime(
            "%d %B a las %H:%M h")
...
def getListadoDocEvento(request):
    ...
    listAdjuntos = AdjuntosEventos.objects.using('spida_web').filter(
        id_evento=idEvento).order_by('-fecha_hora_local')
    for adjunto in listAdjuntos:
        ...
        docAuxiliar[
            'fecha_subida'] = adjunto.fecha_hora_local  #doc.fecha_hora_local.strftime('%d-%m-%Y %H:%M')
...
def getListadoAdjuntosEvento(request):
    ...
    for adjunto in listAdjuntos:
        ...
        adjuntAuxiliar['usuario'] = (get_user_model().objects.get(
            id=adjunto.id_usuario)).get_full_name()
        adjuntAuxiliar[
            'fecha_subida'] = adjunto.fecha_hora_local  # doc.fecha_hora_local.strftime('%d-%m-%Y %H:%M')
...
def getListadoDocumentosAdjuntoEvento(request):
    ...
    listDocs = DocumentacionAdjunto.objects.using('spida_web').filter(
        id_adjunto=idAdjunto).order_by('extension').extra(
            select={
                'id': 'id_documento',
                'adjunto': 'id_adjunto',
                'doc': 'nombre',
...
def getValores24h(request):
    ...
    if IdEst == 7013 and IdCan == 301:
        IdEst = 1110
        IdCan = 301
        estacion = Estaciones.objects.using('spida_web').filter(
            id_estacion=IdEst, monitorizar=1).first()
    else:
        # Obtenemos el dict con la info de la estacion
        estacion = Estaciones.objects.using('spida_web').filter(
            id_estacion=IdEst, monitorizar=1, visualizar=1).first()
    ...
    canales = Canales.objects.using('spida_web').filter(
        id_canal=IdCan).values().first()
    if not canales:
        return JsonResponse(result, safe=False)
    else:
    ...
    data = Valores.objects.using('spida_web').filter(
        id_estacion=IdEst, id_canal=IdCan, fecha_hora_utc__gt=fechahoraUTC_lim)
    data = data.values('fecha_hora_local', 'valor', 'valido')
...
def getPrediccion24h(request):
    ...
    data = data.values('fecha_hora_local', 'valor')
    ...
    canales = Canales.objects.using('spida_web').filter(
        id_canal=IdCanPred).values().first()
    if not canales:
        return JsonResponse(result, safe=False)
...
def getLegendaModelosNumericos(request):
    peticionLegendModelosNumericos = requests.get(
        'http://www.aemet.es/es/api-eltiempo/modelo-harmonie/leyenda/61/PB')
...
def getLegendaRadar(request):
    peticionLegendModelosNumericos = requests.get(
        'http://www.aemet.es/es/api-eltiempo/radar/leyenda-radar/RN1')
    result = peticionLegendModelosNumericos.json()
    return JsonResponse(result, safe=False)
...
def getCamarasEstaciones(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            print("ERROR WEBCAM:", webcam["name"], e)

        if not vid.isOpened():
            return JsonResponse({'resultado': False}, safe=False)

        ret, frame = vid.read()
        cv2.imwrite(
            settings.STATIC_ROOT + 'spidacam/' + webcam["name"] + '.jpg',
            frame)
        ...
        return JsonResponse(
            {
                'resultado': True,
                'imagen': webcam["name"] + '.jpg',
                'descripcion': webcam["descripcion"],
                'fechahora_local': localdt.strftime("%m/%d/%Y %H:%M:%S"),
                'fechahora_utc': utcdt.strftime("%m/%d/%Y %H:%M:%S")
            },
            safe=False)

(function (factory) {
	...
	L.Control.PanelLayers = L.Control.Layers.extend({
		...
		_onInputClick: function () {
			...
			for (i = 0; i < inputsLen; i++) {
				...
				if (input.checked && !this._map.hasLayer(obj.layer)) {
					...
					if(obj.name.includes("Radar")){
						map.addControl(legendPreRadarAemet);
					}
					if(obj.name.includes("Precipitacion en 1 h")){
						map.addControl(timeDimensionControl);
						map.addControl(legendPreMetedoNumericoAemet);			
					}
					if(obj.layer.options.layers=="NZ.RiskZone"){
						num_wms=num_wms+1;
						/*map.addControl(legend);*/
						map.addControl(legendManchasInundacion);
					}
					...
					if(obj.name.includes("Radar")){
						map.removeControl(legendPreRadarAemet);
					}
					if(obj.name.includes("Precipitacion en 1 h")){
						map.removeControl(legendPreMetedoNumericoAemet);
						map.removeControl(timeDimensionControl);
					}
				}
				else{
					if(input.checked && this._map.hasLayer(obj.layer) && obj.layer.options.layers=="NZ.RiskZone"){
						num_wms=num_wms+1;
					}
				}
			}

			//console.log("Total wms activas "+num_wms);
			if(num_wms<=0 && !map.hasLayer(legend)){
				//console.log("Ha entrado a borrar la leyenda");
				/*map.removeControl(legend);	*/
				map.removeControl(legendManchasInundacion);
			} 
		...
		_updateHeight: function (h) {
			...
			/* console.log("La altura del mapa es " + h); */
